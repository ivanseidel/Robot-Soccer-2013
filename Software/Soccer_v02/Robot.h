...
// SENSORS setup
	...
	DistanceInterface 	*Dist[4];
	
	AnalogIn			*analogPins[4];
	bool				lineEnabled[4] = { true, false, false, false };

	DistanceInterface 	*DistLeft,
						*DistRight,
						*DistFront,
						*DistBack;
...
void irqPauseBtn(){
	...
	if(!act || !act->enablePause())
		return;
	
	// Serial.println(pause ? "Pausing..." : "Continuing...");
...
void initRobot(){
	...
	*/
	...
	preLoader.setValue(2, true);
		...
		PC.println("$: GUI Updater Thread registered");
	#endif
    ...
    PC.println("$: Beeper Thread registered");
	preLoader.setValue(5, true);

    // IMU Initialization
    #ifndef DISABLE_IMU
    	IMU = new MPUSensor(Wire1);
	...
	analogReadResolution(10); // Set 10 bits resolution
	...
	#ifndef DISABLE_SENSORS
		analogPins[0] = new AnalogIn(pDIST_1);
		analogPins[1] = new AnalogIn(pDIST_2);
		analogPins[2] = new AnalogIn(pDIST_3);
		analogPins[3] = new AnalogIn(pDIST_4);
		
		DistFront = new EZUltrasonicAnalog(pDIST_6);
		DistRight = new EZUltrasonicAnalog(pDIST_7);
		DistBack = new EZUltrasonicAnalog(pDIST_8);
		DistLeft = new EZUltrasonicAnalog(pDIST_9);
		...
		Compass->setSampleRate(20);
		preLoader.setValue(9, true);

		LineSensor = new LightSensor(pDIST_1);
		preLoader.setValue(10, true);

		Ball = new BallRadar(&BALL_USART);
	#endif
	...
	*/
	...
	bool configResult = MD->setConfig(1, 2); //PWM: 2.5 kHz
	
	Serial.print("PWM configuration result: ");
	Serial.println(configResult);

    // Finished
	PC.println("$: ArduiOS Initialized");
	preLoader.setValue(12, true);
}
...
int getIntensity() {
	...
	for(int i = 0; i < 8; i++){
		...
		xInt += cos(i * M_PI/4.0 - M_PI/2.0) * tempInt;
		yInt += sin(i * M_PI/4.0 - M_PI/2.0) * tempInt;
	}
	return maxIntensity;
}
...
// Filters
GaussianAverage kalmanIntensity(2);
GaussianAverage kalmanDirectionX(2);
GaussianAverage kalmanDirectionY(2);
...
void loop(){
	...
	if (millis() - lastMillis >= 50) {
		...
		for(int i = 0; i < MAX_SENSORS; i++){
			if (IR_DigitalValue[i])
				qttDetecting++;
		}

		// Creates a gaussian object to store the intensity and it's variance
		Gaussian gIntensity = Gaussian(constrain(20 * qttDetecting, 0, 255));
		...
		//Serial.println(qttDetecting);

		double rawXAnalog = 0.0;
		double rawYAnalog = 0.0;

		double normalizeAnalog = sqrt(xInt*xInt + yInt*yInt);
		if(normalizeAnalog > 10){
			double rawXAnalog = (double) xInt / normalizeAnalog;
			double rawYAnalog = (double) yInt / normalizeAnalog;
		}

		rawX += rawXAnalog * kalmanIntensity.process().mean * (1.0*2.0);
		rawY += rawYAnalog * kalmanIntensity.process().mean * (1.0*2.0);
	    /*#else
		...
		//int intensity = constrain(kalmanIntensity.process().mean, 0, 255);
		int intensity = constrain(20 * qttDetecting, 0, 255);
		...
		*/
		...
		uint8_t intByte = (ballNotDetected? 0 : intensity/*(int)constrain(kalmanIntensity.sampleVariance() / 100.0, 0, 255)*/) |  1;
